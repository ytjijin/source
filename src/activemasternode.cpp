...
//
// Bootup the masternode, look for a 20000 YTGM input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in ytgm.conf */
...
{
    CYTGMAddress address(collateralAddress);
