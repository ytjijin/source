...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\YTGM
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\YTGM
    // Mac: ~/Library/Application Support/YTGM
    // Unix: ~/.YTGM
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "YTGM";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ytgm.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ytgm.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ytgmd.pid"));
