...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <ytgmaddress> <amount>\n"
            "ytgmaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <ytgmaddress> <amount>\n"
            "ytgmaddress, denominate, or auto (AutoDenominate)"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in ytgm.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in ytgm.conf\n"
                ...
                "  vote-many    - Vote on a YTGM initiative\n"
                "  vote         - Vote on a YTGM initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CYTGMSecret(secret).ToString();
    }
...
Value masternodelist(const Array& params, bool fHelp)
{
				...
				&& strMode != "score"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a YTGM initiative (can be additionally filtered, partial match)\n"
