...
const string strMessageMagic = "YTGM Signed Message:\n";
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue)
{
    ...
    if (nNow <= MN_START_TIMESTAMP){
         ret = 0; //0%
         if (fDebug)
            LogPrintf("GetMasternodePayment: time is too early, GetTime(): %d, LIMIT: %d, blockValue: %d \n", nNow, MN_START_TIMESTAMP, ret);
    }else{
         ret = blockValue; //100%
         if (fDebug)
            LogPrintf("GetMasternodePayment: blockValue: %d \n", ret);
    }
