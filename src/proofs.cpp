...
int64_t GetCoinstakeValue(int64_t nCoinAge, CAmount nFees, int nHeight)
{
    ...
    int64_t nThreeMonthTime = 60 * 60 * 24 * 30 * 3 + 1;

    int64_t nNow = GetTime();

    if (nNow <= MN_START_TIMESTAMP){
         nSubsidy = 0; //0%
         if (fDebug)
            LogPrintf("GetCoinstakeValue: time is too early, GetTime(): %d, LIMIT: %d, nSubsidy: %d \n", nNow, MN_START_TIMESTAMP, nSubsidy);
    } else {
        int n = (nNow - MN_START_TIMESTAMP) / nThreeMonthTime;
        if (n > 0) {
            nSubsidy = nSubsidy * pow(0.9, n);
            if (fDebug)
                LogPrintf("GetCoinstakeValue: n: %d, nSubsidy: %d \n", n, nSubsidy);
        }
    }
