...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeYTGMAmount"))
        settings.setValue("nAnonymizeYTGMAmount", 1000);
    nAnonymizeYTGMAmount = settings.value("nAnonymizeYTGMAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeYTGMAmount"))
        SoftSetArg("-anonymizeytgmamount", settings.value("nAnonymizeYTGMAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeYTGMAmount:
            return QVariant(nAnonymizeYTGMAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeYTGMAmount:
            nAnonymizeYTGMAmount = value.toInt();
            settings.setValue("nAnonymizeYTGMAmount", nAnonymizeYTGMAmount);
            emit AnonymizeYTGMAmountChanged(nAnonymizeYTGMAmount);
            break;
        default:
            break;
        }
