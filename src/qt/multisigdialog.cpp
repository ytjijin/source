...
void MultisigDialog::on_saveMultisigAddressButton_clicked()
{
    ...
    if(!wallet->HaveCScript(scriptID))
        wallet->AddCScript(script);
    if(!wallet->mapAddressBook.count(CYTGMAddress(address).Get()))
        wallet->SetAddressBookName(CYTGMAddress(address).Get(), label);
}
    ...
    for(int i = 0; i < ui->outputs->count(); i++)
    {
        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                CYTGMAddress address(recipient.address.toStdString());
